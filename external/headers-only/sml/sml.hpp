...
struct pool_type : pool_type_base {
  explicit pool_type(T object)
      : value{object} {}
  template <class TObject>
  pool_type(init i, TObject object)
      : value{i, object} {}
  T value;
};
template <class T>
...
struct pool : pool_type<Ts>... {
  ...
  pool(init, const pool<TArgs...> &p)
      : pool_type<Ts>(try_get<aux::remove_const_t<aux::remove_reference_t<Ts>>>(&p))... {}
  template <class... TArgs>
...
#if defined(_MSC_VER) && !defined(__clang__)
constexpr int max_impl() {
  return 0;
}
constexpr int max_impl(int r) {
  return r;
}
constexpr int max_impl(int r, int i) {
  return r > i ? r : i;
}
constexpr int max_impl(int r, int i, int ints...) {
...
struct zero_wrapper<R (TBase::*)(TArgs...), T> {
  explicit zero_wrapper(R (TBase::*ptr)(TArgs...))
      : ptr{ptr} {}
  auto operator()(TBase &self, TArgs... args) {
...
struct zero_wrapper<R (TBase::*)(TArgs...) const, T> {
  explicit zero_wrapper(R (TBase::*ptr)(TArgs...) const)
      : ptr{ptr} {}
  auto operator()(TBase &self, TArgs... args) {
    return (self.*ptr)(args...);
  }
...
struct zero_wrapper<R (*)(TArgs...), T> {
  explicit zero_wrapper(R (*ptr)(TArgs...))
      : ptr{ptr} {}
...
struct zero_wrapper<R (TBase::*)(TArgs...) noexcept, T> {
  explicit zero_wrapper(R (TBase::*ptr)(TArgs...) noexcept)
      : ptr{ptr} {}
  auto operator()(TBase &self, TArgs... args) {
    return (self.*ptr)(args...);
  }
...
struct zero_wrapper<R (TBase::*)(TArgs...) const noexcept, T> {
  explicit zero_wrapper(R (TBase::*ptr)(TArgs...) const noexcept)
      : ptr{ptr} {}
  auto operator()(TBase &self, TArgs... args) {
    return (self.*ptr)(args...);
  }
...
struct zero_wrapper<R (*)(TArgs...) noexcept, T> {
  explicit zero_wrapper(R (*ptr)(TArgs...) noexcept)
      : ptr{ptr} {}
  auto operator()(TArgs... args) {
    return (*ptr)(args...);
  }
...
#if defined(_MSC_VER) && !defined(__clang__)
  return detail::get_type_name<T, 39>(__FUNCSIG__,
                                      make_index_sequence<sizeof(__FUNCSIG__) - 39 - 8>{});
#elif defined(__clang__)
  return detail::get_type_name<T, 63>(__PRETTY_FUNCTION__,
                                      make_index_sequence<sizeof(__PRETTY_FUNCTION__) - 63 - 2>{});
#elif defined(__GNUC__)
  return detail::get_type_name<T, 68>(__PRETTY_FUNCTION__,
                                      make_index_sequence<sizeof(__PRETTY_FUNCTION__) - 68 - 2>{});
#endif
}
...
public:
  ...
  explicit queue_event_call(const call_t &call)
      : call{call} {}
  call_t call{};
};
...
template <class... Ts>
using get_all_events = aux::join_t<typename get_all_events_impl<typename Ts::src_state,
                                                                typename Ts::dst_state,
...
using get_initial_states =
    aux::join_t<typename aux::conditional<Ts::initial,
...
template <class... Ts>
using get_non_empty_t = aux::join_t<typename aux::conditional<aux::is_empty<Ts>::value,
                                                              aux::type_list<>,
...
struct sub_sm<sm_impl<sm_policy<T>>> {
  ...
  static sm_impl<sm_policy<T, aux::identity<U>, TPolicies...>> &
  ...
  static sm_impl<sm_policy<T, TPolicies...>> &
  ...
  template <class... TPolicies>
  static const sm_impl<sm_policy<T, TPolicies...>> &
...
struct sub_sm<sm_impl<sm_policy<T, aux::identity<U>>>> {
  ...
  template <class... TPolicies>
  static const sm_impl<sm_policy<T, aux::identity<U>, TPolicies...>> &
...
struct transitions<T, Ts...> {
  ...
  static bool execute(
      const TEvent &event, SM &sm, TDeps &deps, TSubs &subs, typename SM::state_t &current_state) {
...
struct transitions_sub<sm<TSM>, T, Ts...> {
  ...
  static bool execute(const anonymous &event,
                      SM &sm,
                      TDeps &deps,
                      TSubs &subs,
  ...
  static bool execute_impl(const back::on_entry<_, TEvent> &event,
                           SM &sm,
                           TDeps &deps,
                           TSubs &subs,
                           typename SM::state_t &current_state) {
  ...
  template <class _, class TEvent, class SM, class TDeps, class TSubs>
  static bool execute_impl(const back::on_exit<_, TEvent> &event,
                           SM &sm,
                           TDeps &deps,
                           TSubs &subs,
                           typename SM::state_t &current_state) {
...
struct get_mapping<event<E>, event<E>, T, R>
    : aux::type_list<event_mappings<
          E,
          aux::apply_t<unique_mappings_t, aux::join_t<typename R::types, typename T::types>>>> {};
...
struct extend_mapping
    : aux::join_t<typename get_mapping<typename T::element_type, typename Ts::element_type, T, Ts>::
                      type...> {};
template <class T, class... Ts>
...
struct jump_table {
  template <int,
            ...
            class TSubs>
  static bool
  dispatch(sm_impl &, State &, const TEvent &, TDeps &, TSubs &, const aux::type_list<> &) {
    return false;
  }
  template <int,
            class TMappings,
            ...
            class TSubs,
            class... TStates>
  static bool dispatch(sm_impl &self,
                       State &current_state,
                       const TEvent &event,
                       TDeps &deps,
                       TSubs &subs,
...
struct branch_stm {
  ...
  static bool dispatch(sm_impl &self,
    ...
    return current_state == N
             ? get_state_mapping_t<TState, TMappings, typename sm_impl::has_unexpected_events>::
                   template execute<TEvent, sm_impl, TDeps, TSubs>(
                       event, self, deps, subs, current_state)
             : dispatch<N + 1, TMappings>(
  ...
  static bool dispatch(sm_impl &self,
    ...
    case N:
      return get_state_mapping_t<TState, TMappings, typename sm_impl::has_unexpected_events>::
          template execute<TEvent, sm_impl, TDeps, TSubs>(event, self, deps, subs, current_state);
    default:
      return dispatch<N + 1, TMappings>(
          self, current_state, event, deps, subs, aux::type_list<TStates...>{});
    }
  }
};
...
struct fold_expr {
  template <class TMappings,
            ...
            class TSubs,
            class... TStates>
  static bool dispatch_impl(sm_impl &self,
                            State &current_state,
  ...
  static bool dispatch(sm_impl &self,
    ...
    return dispatch_impl<TMappings>(self,
                                    current_state,
                                    aux::make_index_sequence<sizeof...(TStates)>{},
                                    event,
                                    deps,
                                    subs,
                                    states);
...
void log_state_change(const aux::type<TLogger> &,
                      TDeps &deps,
                      const TSrcState &,
                      const TDstState &) {
...
void log_action(const aux::type<TLogger> &,
                TDeps &deps,
...
void log_guard(const aux::type<TLogger> &,
               TDeps &deps,
               const TGuard &guard,
               const TEvent &event,
               bool result) {
  return static_cast<aux::pool_type<TLogger &> &>(deps).value.template log_guard<SM>(
      guard, event, result);
}
template <class SM, class TLogger, class TDeps, class TGuard, class TEvent>
void log_guard(const aux::type<TLogger> &,
...
struct thread_safe : aux::pair<thread_safety_policy__, thread_safe<TLock>> {
  ...
  auto create_lock() {
    struct lock_guard {
      explicit lock_guard(TLock &lock)
          : lock_{lock} {
        lock_.lock();
      }
      ~lock_guard() {
...
struct sm_policy {
  ...
  using thread_safety_policy = decltype(
      get_policy<no_policy, policies::thread_safety_policy__>((aux::inherit<TPolicies...> *)0));
  using defer_queue_policy = decltype(
      get_policy<no_policy, policies::defer_queue_policy__>((aux::inherit<TPolicies...> *)0));
  using process_queue_policy = decltype(
      get_policy<no_policy, policies::process_queue_policy__>((aux::inherit<TPolicies...> *)0));
  using logger_policy =
  ...
  using dispatch_policy = decltype(get_policy<default_dispatch_policy, policies::dispatch_policy__>(
...
template <class TSM>
struct sm_impl
    : aux::conditional_t<aux::is_empty<typename TSM::sm>::value, aux::none_type, typename TSM::sm> {
  ...
  using has_history_states   = aux::integral_constant<bool,
                                                    aux::size<initial_states_t>::value
  ...
  using events_t =
      aux::apply_t<aux::unique_t,
                   aux::join_t<sub_internal_events_t, aux::apply_t<get_all_events, transitions_t>>>;
  ...
  sm_impl(aux::init, const TPool &p)
      : sm_impl{p, aux::is_empty<sm_t>{}} {}
  template <class TPool>
  sm_impl(const TPool &p, aux::false_type)
      : sm_t{aux::try_get<sm_t>(&p)}
      , transitions_{(*this)()} {
    initialize(typename sm_impl<TSM>::initial_states_t{});
  }
  template <class TPool>
  sm_impl(const TPool &p, aux::true_type)
      : transitions_{aux::try_get<sm_t>(&p)()} {
    initialize(typename sm_impl<TSM>::initial_states_t{});
  }
  ...
  bool process_event(const TEvent &event, TDeps &deps, TSubs &subs) {
    ...
    do {
      ...
      } while (process_defer_events(
          deps, subs, handled, aux::type<defer_queue_t<TEvent>>{}, events_t{}));
...
#else
    return process_event_noexcept<get_event_mapping_t<get_generic_t<TEvent>, mappings>>(
  ...
  bool process_internal_events(const TEvent &event, TDeps &deps, TSubs &subs) {
    ...
    return process_event_impl<get_event_mapping_t<get_mapped_t<TEvent>, mappings>>(
        event, deps, subs, states_t{}, aux::make_index_sequence<regions>{});
#else
    return process_event_noexcept<get_event_mapping_t<get_mapped_t<TEvent>, mappings>>(
        event, deps, subs, has_exceptions{});
#endif
  }
  template <class TEvent,
  ...
  template <class TEvent,
            ...
            class TSubs,
            __BOOST_SML_REQUIRES(aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value)>
  bool process_internal_generic_event(const TEvent &event,
                                      TDeps &deps,
                                      TSubs &subs,
  ...
  template <class TEvent,
            ...
            __BOOST_SML_REQUIRES(aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value
                                 && !aux::is_base_of<get_mapped_t<TEvent>, events_ids_t>::value)>
  bool
  process_internal_event(const TEvent &event, TDeps &deps, TSubs &subs, state_t &current_state) {
  ...
  bool process_event_impl(const TEvent &event,
                          TDeps &deps,
                          TSubs &subs,
                          const aux::type_list<TStates...> &states,
  ...
  bool
  process_event_noexcept(const TEvent &event, TDeps &deps, TSubs &subs, aux::false_type) noexcept {
    return process_event_impl<TMappings>(
        event, deps, subs, states_t{}, aux::make_index_sequence<regions>{});
  }
  template <class TMappings, class TEvent, class TDeps, class TSubs>
  bool process_event_noexcept(const TEvent &event,
                              TDeps &deps,
                              TSubs &subs,
                              state_t &current_state,
  ...
  bool process_defer_events(TDeps &,
                            TSubs &,
                            const bool,
  ...
  bool process_defer_events(TDeps &deps,
                            TSubs &subs,
                            const bool handled,
  ...
  template <class TDeps, class TSubs, class... TEvents>
  bool process_queued_events(TDeps &,
                             TSubs &,
                             const aux::type<no_policy> &,
                             const aux::type_list<TEvents...> &) {
    return false;
  ...
  bool process_event_no_queue(TDeps &deps, TSubs &subs, const void *data) {
    ...
    return process_event_impl<get_event_mapping_t<TEvent, mappings>>(
        event, deps, subs, states_t{}, aux::make_index_sequence<regions>{});
#else
    return process_event_noexcept<get_event_mapping_t<TEvent, mappings>>(
        event, deps, subs, has_exceptions{});
#endif
  }
  template <class TDeps, class TSubs, class TDeferQueue, class... TEvents>
  bool process_queued_events(TDeps &deps,
                             TSubs &subs,
                             const aux::type<TDeferQueue> &,
                             const aux::type_list<TEvents...> &) {
    using dispatch_table_t   = bool (sm_impl::*)(TDeps &, TSubs &, const void *);
...
private:
  using sm_all_t =
      aux::apply_t<get_non_empty_t,
                   aux::join_t<aux::type_list<sm_t>, aux::apply_t<get_sm_t, state_machines>>>;
  using sub_sms_t = aux::apply_t<
      aux::pool,
      typename convert_to_sm<TSM,
                             aux::apply_t<aux::unique_t, aux::apply_t<get_sub_sms, states>>>::type>;
  using deps   = aux::apply_t<merge_deps, transitions_t>;
  using deps_t = aux::apply_t<
      aux::pool,
      aux::apply_t<aux::unique_t,
                   aux::join_t<deps, sm_all_t, logger_dep_t, aux::apply_t<merge_deps, sub_sms_t>>>>;
  struct events_ids : aux::apply_t<aux::inherit, events> {};

public:
  sm()
      : deps_{aux::init{}, aux::pool<>{}}
      , sub_sms_{aux::pool<>{}} {
  ...
  template <class T = aux::identity<sm_t>,
            class TVisitor,
...
template <class T, class TEvent, class TSM, class TDeps>
decltype(auto)
get_arg(const aux::type<const TEvent &> &, const back::on_entry<T, TEvent> &event, TSM &, TDeps &) {
  return event.event_;
}
template <class T, class TEvent, class TSM, class TDeps>
decltype(auto)
get_arg(const aux::type<const TEvent &> &, const back::on_exit<T, TEvent> &event, TSM &, TDeps &) {
  return event.event_;
}
template <class T, class TEvent, class TSM, class TDeps>
decltype(auto) get_arg(const aux::type<const TEvent &> &,
                       const back::exception<T, TEvent> &event,
                       TSM &,
                       TDeps &) {
...
template <class... TEvents, class TEvent, class TSM, class TDeps>
decltype(auto)
get_arg(const aux::type<back::process<TEvents...>> &, const TEvent, TSM &sm, TDeps &) {
...
struct call<TEvent, aux::type_list<Ts...>, TLogger> {
  ...
  static auto
  execute_impl(const aux::type<bool> &, T object, const TEvent &event, SM &sm, TDeps &deps) {
  ...
  template <class TSM, class T, class SM, class TDeps>
  static auto
  execute_impl(const aux::type<void> &, T object, const TEvent &event, SM &sm, TDeps &deps) {
    back::policies::log_action<TSM>(aux::type<TLogger>{}, deps, object, event);
...
private:
  ...
  void for_all(
      const aux::index_sequence<Ns...> &, const TEvent &event, TSM &sm, TDeps &deps, TSubs &subs) {
#if defined(__cpp_fold_expressions)
...
private:
  ...
  auto for_all(
    ...
    auto result = true;
    (void)aux::swallow{0,
                       (result &= call<TEvent, args_t<Ts, TEvent>, typename TSM::logger_t>::execute(
                            aux::get_by_id<Ns>(&g), event, sm, deps, subs),
                        0)...};
    return result;
#endif
...
struct sm : back::sm<back::sm_policy<T, TPolicies...>> {
  constexpr sm(T t)
      : back::sm<back::sm_policy<T, TPolicies...>>::sm{t} {}
...
template <class T>
auto transitional_impl(T &&t) -> aux::always<typename T::dst_state,
...
struct ignore<E, aux::type_list<Ts...>> {
  ...
  struct non_events {
    using type = aux::conditional_t<
        aux::is_same<back::get_event_t<E>, aux::remove_const_t<aux::remove_reference_t<T>>>::value
...
void update_current_state(SM &,
                          ...
                          const TSrcState &,
                          const TDstState &) {
  back::policies::log_state_change<typename SM::sm_t>(aux::type<typename SM::logger_t>{},
                                                      deps,
...
void update_current_state(SM &,
  ...
  back::policies::log_state_change<typename SM::sm_t>(aux::type<typename SM::logger_t>{},
                                                      ...
                                                      aux::string<T>{});
  current_state = new_state;
  update_composite_states<back::sm_impl<T>>(subs,
...
struct transition<state<internal>, state<S2>, front::event<E>, G, A> {
  ...
  bool
  execute(const TEvent &event, SM &sm, TDeps &deps, TSubs &subs, typename SM::state_t &, Ts &&...) {
    if (call<TEvent, args_t<G, TEvent>, typename SM::logger_t>::execute(g, event, sm, deps, subs)) {
      call<TEvent, args_t<A, TEvent>, typename SM::logger_t>::execute(a, event, sm, deps, subs);
      return true;
    }
