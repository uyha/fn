...
TEST_CASE("Member functions can be invoke with the object as its first argument") {
  ...
  CHECK(fn<&A::rvalue_fn_const_noexcept>{}(std::move(one)) == 1);
}
...
TEST_CASE("noexcept should be propagated correctly") {
  ...
  CHECK(noexcept(fn<&A::fn_noexcept>{}(zero)));
  CHECK(noexcept(fn<&A::fn_const_noexcept>{}(zero)));
  CHECK(noexcept(fn<&A::fn_const_noexcept>{}(one)));
  CHECK(noexcept(fn<&A::fn_noexcept>{}(A{0})));
  CHECK(noexcept(fn<&A::fn_const_noexcept>{}(A{0})));
  CHECK(noexcept(fn<&A::rvalue_fn_noexcept>{}(A{0})));
  CHECK(noexcept(fn<&A::rvalue_fn_const_noexcept>{}(A{0})));
  CHECK(noexcept(fn<&A::fn_const_noexcept>{}(std::move(one))));
  CHECK(noexcept(fn<&A::rvalue_fn_const_noexcept>{}(std::move(one))));
}
#endif
